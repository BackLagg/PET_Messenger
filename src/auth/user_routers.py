...
async def create_or_update_user_info(
    ...
    query = select(UserInfo).where(UserInfo.user_id == user.id)
    result = await session.execute(query)  # Вызов метода execute на сессии
    user_info_record = result.scalars().first()
...
async def forgot_password(
        forgot_password_data: ForgotPasswordRequest,  # Заменяем email на Pydantic схему
        user_manager: UserManager = Depends(get_user_manager),
):
...
async def reset_password(
    reset_password_data: ResetPasswordRequest,  # Используем схему ResetPasswordRequest
    user_manager: UserManager = Depends(get_user_manager),
):
    try:
